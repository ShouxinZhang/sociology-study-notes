...
| `音乐/tools/qqmusic_export/` | 复用 QQ 音乐既有登录态的隔离采集工具；按客户端会话、运行时桥接、数据校验和 JSON/CSV/Markdown/M3U 导出分层 |
| `音乐/tests/` | 使用无凭据小样本验证歌单完整性、认证字段清理与多格式输出 |
| `音乐/exports/` | 按导出时间分批保存本地数据；每批提供机器可恢复 JSON、表格 CSV、逐歌单 Markdown 阅读视图与 M3U；内容默认由目录级 `.gitignore` 排除，避免大批个人数据进入 Git |
